-- Discover > Venue calendar dots: when p_region_only is true, return viewport-scoped
-- venue_events only. Official public.games dates remain included when p_region_only is false.
...
AS $$
  ...
  combined AS (
    ...
    UNION ALL
    SELECT od.d
    FROM official_dates od
    CROSS JOIN ids_nonempty p
    WHERE NOT p.region_only
  )
...
COMMENT ON FUNCTION public.gameon_calendar_dot_dates(
  ...
  'Read-only aggregation for GameON calendar dot dates. '
  'Returns distinct dates from active public.venue_events and, when p_region_only is false, '
  'public.games (official schedule). When p_region_only is true, only viewport-scoped venue_events '
  'qualify (venue_id = ANY(p_venue_ids), or legacy owner_email / venue_name for rows with venue_id IS NULL). '
  'When p_region_only is false, all active venue_events in the date range qualify. '
  'Sport filter applies unless p_sport is null, empty, or All (case-insensitive). '
