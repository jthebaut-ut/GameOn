-- Align Suggested Fans RPC + public profile identity with soft-deleted / undiscoverable fan eligibility.
...
COMMENT ON FUNCTION public.get_profile_friend_suggestions(
  ...
  'Ranked privacy-safe fan suggestions. Excludes self, friends/pending/declined, blocks, business, disabled, undiscoverable, soft-deleted, and suggested_fan_dismissals. Eligible discoverable fans without signal matches receive a low-score recent_activity fallback.';

-- Public profile RPC: privacy-safe home city fields for fan identity strip.
-- Privacy rule: home city/region/country are returned only when show_home_city = true
-- AND home_city is non-empty. Never expose GPS, coordinates, or email.
...
COMMENT ON FUNCTION public.get_public_fan_identity_profile(uuid) IS
  'Public fan profile payload for authenticated viewers. Home city fields are privacy-gated: returned only when show_home_city=true and home_city is set.';
