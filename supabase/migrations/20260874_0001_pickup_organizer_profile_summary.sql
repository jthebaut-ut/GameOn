-- =============================================================================
-- 20260874 — Pickup organizer profile summary (hosted count + rating aggregates)
-- =============================================================================
-- Adds a single-round-trip aggregate for profile surfaces:
--   pickup_games_hosted_count
--   pickup_organizer_average_rating  (NULL when no ratings)
--   pickup_organizer_rating_count
--
-- Embeds those fields into get_public_fan_identity_profile so public profiles
-- do not need a second RPC for organizer rating stats.
--
-- Hosted-game eligibility (matches existing product retention rules):
--   - Count rows in public.pickup_games created by the user
--   - status IN ('active', 'removed', 'expired') — the only valid statuses
--   - Soft-cancelled games remain status='removed' until purge; they count
--     because the user did host them and ratings/history are retained
--   - Hard-purged / deleted rows are absent and are not counted
--   - No archived/test status exists on this table
--
-- Rating eligibility:
--   - public.pickup_game_creator_ratings rows for creator_user_id
--   - UNIQUE (pickup_game_id, rater_user_id) → count(*) = distinct rating records
--   - Updates overwrite the same row (no double-count)
--
...
BEGIN
  ...
  END IF;
  IF to_regprocedure('public.pickup_creator_public_rating_stats(uuid)') IS NULL THEN
    v_missing := v_missing || ARRAY['function public.pickup_creator_public_rating_stats(uuid)'];
  END IF;
  IF cardinality(v_missing) > 0 THEN
    RAISE EXCEPTION '20260874 preflight failed; missing: %', array_to_string(v_missing, ', ');
  END IF;
END $$;

-- ---------------------------------------------------------------------------
-- Shared aggregate (own profile + embedded in public identity RPC)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.pickup_organizer_profile_summary(p_user_id uuid)
RETURNS TABLE (
...
COMMENT ON FUNCTION public.pickup_organizer_profile_summary(uuid) IS
  'Profile aggregates: hosted pickup game count + organizer average/count. No reviewer identities or feedback.';
...
-- ---------------------------------------------------------------------------
-- Public identity profile: include organizer aggregates in the same jsonb
-- (visibility / block / discoverability gates unchanged from 20260871)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  INTO v_pickup_hosted, v_organizer_avg, v_organizer_rating_count
...
COMMENT ON FUNCTION public.get_public_fan_identity_profile(uuid) IS
  'Public fan identity profile for authenticated viewers. Includes Fan XP and pickup organizer hosted/rating aggregates when visible.';
...
-- SELECT jsonb_pretty(public.get_public_fan_identity_profile('<other-user-uuid>'::uuid));
