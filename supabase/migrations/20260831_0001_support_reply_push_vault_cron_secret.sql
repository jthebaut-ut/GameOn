-- Support reply push: pg_net must send x-cron-secret from Vault (Edge secrets are not visible to Postgres).
-- Manual Vault setup: docs/ops/Support_Reply_Push_Vault_Cron_Secret.md
...
AS $$
  SELECT
    EXISTS (
      ...
      WHERE name = 'PRO_SCORE_PUSH_WORKER_CRON_SECRET'
...
REVOKE ALL ON FUNCTION public.support_reply_push_vault_auth_status() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.support_reply_push_vault_auth_status() TO service_role;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE
      'support reply push: Vault PRO_SCORE_PUSH_WORKER_CRON_SECRET missing; '
...
COMMENT ON FUNCTION public.queue_support_reply_push_notification(uuid, uuid) IS
  'Async notify-support-reply invoke. Bearer unchanged (Vault service role). Auth via x-cron-secret when Vault has PRO_SCORE_PUSH_WORKER_CRON_SECRET.';

COMMENT ON FUNCTION public.support_reply_push_vault_auth_status() IS
  'Ops check: Vault readiness for support-reply pg_net auth (lengths only, no secret values).';
