-- Friend exception for Discover My Profile OFF:
-- accepted user↔user friends can open public fan identity / Home Watch Spot,
-- while Suggested Fans, Nearby, and public discovery remain discoverable-only.
...
COMMENT ON FUNCTION public.get_public_fan_identity_profile(uuid) IS
  'Public fan profile for authenticated viewers. Visible when discoverable_by_fans OR accepted user↔user friendship. Blocks and account restrictions still apply. Suggested Fans / Nearby stay discoverable-only.';
...
COMMENT ON FUNCTION public.get_public_fan_home_crowd(uuid) IS
  'Public Home Watch Spot payload. Same discoverable OR accepted-friend visibility as get_public_fan_identity_profile.';

COMMENT ON FUNCTION public.get_public_fan_home_crowd_pointer(uuid) IS
  'Public Home Watch Spot pointer. Same discoverable OR accepted-friend visibility as get_public_fan_identity_profile.';
