-- Mutual-friend avatar identities for public Fan Profile (production-derived patch).
-- Baseline: exact production get_public_fan_identity_profile as of pre-apply capture
--   /tmp/get_public_fan_identity_profile_remote_before_20260862.sql (md5 aad423389393508cdec486b74c196a28)
--   which matches local 20260860 body.
--
-- Approved deltas ONLY in mutual_fan_avatars:
--   1) Remove discoverable_by_fans requirement for mutual avatar rows.
--   2) Exclude disabled / soft-deleted / deleted-email / business / blocked mutual profiles.
--
-- Explicitly unchanged:
--   mutual_fans_count uses count(*)
--   mutual avatar LIMIT 4 and candidate-selection shape
--   no new DISTINCT / ORDER BY on the mutual candidate subquery
--   target visibility gate, payload keys, Suggested Fans / Nearby untouched
--
-- Grants: REVOKE PUBLIC + GRANT authenticated only. Do not touch anon EXECUTE
--   (production currently has anon EXECUTE; intentionality unresolved — preserve as-is).
-- Local migration — apply only when explicitly authorized. Do not db push.
...
-- Preserve production grant posture: PUBLIC revoked, authenticated granted.
-- Do NOT revoke or grant anon here — CREATE OR REPLACE preserves existing anon EXECUTE.
...
COMMENT ON FUNCTION public.get_public_fan_identity_profile(uuid) IS
  'Public fan profile for authenticated viewers. Target visibility unchanged: discoverable_by_fans OR accepted user↔user friendship. Mutual-friend avatar rows may include accepted friends of the viewer even when those friends have Discover My Profile OFF; strangers remain hidden. Blocks and account restrictions still apply. Suggested Fans / Nearby stay discoverable-only.';
