...
/// FanGeo in-app share sheet for favorite spots / venues — same recipient multi-select as pickup/profile share.
struct ShareVenueSheet: View {
    let venue: BarVenue
    ...
    private var sharePayload: VenueSharePayload? {
        VenueShareMessage.payload(
            from: venue,
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                List {
                    ...
                    Section {
                        ...
                        Text(L10n.t("share_favorite_spot_send_to", languageCode: languageCode))
                    } footer: {
                        Text(
                            String(
            ...
            .navigationTitle(L10n.t("share_favorite_spot_sheet_title", languageCode: languageCode))
    ...
    private func sendShare() async {
        ...
        guard let payload = sharePayload else {
            await MainActor.run { errorText = L10n.t("share_favorite_spot_unavailable", languageCode: languageCode) }
            return
        }
        ...
        let body = VenueShareMessage.encodeBody(payload: payload)
        guard body.count <= 1000 else {
#if DEBUG
            print("[VenueShareDebug] payloadTooLarge bytes=\(body.count)")
#endif
...
#if DEBUG
        print("[VenueShareDebug] venueId=\(payload.venueId.uuidString.lowercased()) name=\(payload.name)")
        print("[VenueShareDebug] recipientCount=\(recipients.count)")
#endif
        ...
        if let err = await chatViewModel.shareFanProfileMessage(
            ...
            await MainActor.run {
                errorText = L10n.t("share_favorite_spot_failed", languageCode: languageCode)
#if DEBUG
                print("[VenueShareDebug] sendFailed localized=\(err)")
#endif
            }
            return
        }
        ...
        await MainActor.run {
            mapViewModel.showSocialActionToast(L10n.t("share_favorite_spot_success", languageCode: languageCode), isError: false)
            dismiss()
        }
    }
}

/// Compact Share control for Going → Favorite Spots venue cards.
struct VenueShareActionButton<LabelContent: View>: View {
    let venue: BarVenue
    @ObservedObject var mapViewModel: MapViewModel
    ...
    private var canShare: Bool {
        mapViewModel.isAuthenticatedForSocialFeatures
            ...
            && VenueShareMessage.payload(from: venue, sharedByDisplayName: mapViewModel.currentUserDisplayName) != nil
    }
    ...
    var body: some View {
        Group {
            if canShare {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(L10n.t("share_favorite_spot_a11y_label", languageCode: languageCode))
                .accessibilityHint(L10n.t("share_favorite_spot_a11y_hint", languageCode: languageCode))
                .sheet(isPresented: $showShareSheet) {
                    ShareVenueSheet(venue: venue, mapViewModel: mapViewModel)
